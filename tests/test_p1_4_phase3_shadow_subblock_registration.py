...
The production config carries shadow sub-blocks that had no canonical
leaves and no nested validation spec, plus one sized leaf that was
spec-checked but missing its canonical default:
...
The record-only arms and stop_tuning_shadow default to
...
2026-09-21 cleanup: two record-only arms ``early_breakout_shadow`` and
``short_only_shadow`` were retired with their orphan recorder/reconcile
pipeline and removed here.
"""
...
# ── canonical registration of the shadow sub-blocks ──────────────────
