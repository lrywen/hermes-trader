...
from hermes_trader.agents.risk_gates import GateContext, eval_all_gates, side_adjusted_own_gap

# P1-1 (executor decomposition ①): shadow / gray-release / durable-audit
# recorders were extracted to the agents.shadow subpackage. They are imported
# back here to preserve executor's attribute + call surface (including the
# two confidence-decay singletons, which are the same objects, not copies).
# F401 is intentional for every name below: this is a compatibility re-export.
from hermes_trader.agents.shadow import (  # noqa: F401
    _ATR_CALIB_MODES,
...
# P1-1 (executor decomposition ①): force-override durable audit
# (_FORCE_OVERRIDE_CONFIG_KEYS / _record_force_override_armed), the
# entry-path shadow recorders (pullback / risk-tuning / per-coin regime probe
# / short-only / early-breakout + their path constants), the ATR-regime
# calibration gray release, sizing-v2 config/path/recorder, and the AI
# confidence-freshness decay (incl. its onset/lock singletons) moved to
# hermes_trader.agents.shadow.*. They are imported back below so every
# executor attribute and call site keeps working unchanged.


# Pullback-long / risk-tuning / short-only / early-breakout entry shadow
# recorders and their path constants moved to hermes_trader.agents.shadow.
...
# ATR regime calibration moved to hermes_trader.agents.shadow.atr_calib.


# Sizing v2 gray-release config/path/recorder moved to
# hermes_trader.agents.shadow.sizing_v2.


# AI-confidence freshness decay moved to
# hermes_trader.agents.shadow.confidence_decay.
...
def _v1_stop_width(dsl: dict[str, Any], leverage: float) -> float:
    ...
    top-level DSL stop (live sets 1.0% / 15% at 10x → 1.0% stop). Computed up
    front so the v2 shadow path can log the v1-vs-v2 comparison while still
    sizing on v1. Pure leaf extracted in the P1-1 step ③ phase split.
