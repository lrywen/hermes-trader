#!/usr/bin/env python3
"""Historical backfiller for the trend_filter_200ma shadow arm.
...
This arm has NO live reconcile script — the grading geometry is defined here,
mirroring the daily_extension_cap convention (same counterfactual question:
"the gate vetoed this long signal; where did price go?"):
  * only records with ``trend_would_block is True`` are counterfactual
    trades (a vetoed below-200SMA long without daily-mover bypass);
  * entry = the gate-evaluated ``price`` field stamped on the record;
  * timestamp-anchored fixed forward grid 24h/72h/168h on closed 1h bars;
    gross pct (long-only), win/loss on the 72h close (primary).

Scope / safety contract (trend_filter_200ma ONLY):
  * dry-run by default; ``--write`` writes an ISOLATED product
    (``trend_filter_shadow.backfill.jsonl``), never the live JSONL —
    use ``--out`` to redirect;
...
Usage:
    python3 scripts/backfill_trend_filter_historical.py          # dry-run
    python3 scripts/backfill_trend_filter_historical.py --write
"""
...
DEFAULT_FILE = "/data/trend_filter_shadow.jsonl"
DEFAULT_OUT = "/data/trend_filter_shadow.backfill.jsonl"
FILE = os.environ.get("HERMES_TREND_FILTER_SHADOW_FILE", DEFAULT_FILE)

ARM = "trend_filter_200ma"
INTERVAL = "1h"
FWD_HOURS = (24, 72, 168)
PRIMARY_FWD_HOUR = 72
...
_TERMINAL = MATURE_OUTCOMES + (
    "no_coin", "no_entry_px", "bad_timestamp", "no_future_bars", "side_skip")
        ...
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                # Dedupe across rotations: (coin, signal ts) identity — the
                # trend_filter record carries no entry_px.
                rid = (r.get("coin"), r.get("timestamp"), r.get("price"))
                if rid in seen_ids:
                    continue
                seen_ids.add(rid)
...
def grade_record(rec: dict[str, Any], *, as_of_ms: Optional[int] = None,
    ...
    Returns the terminal/non-terminal outcome string."""
    coin = rec.get("coin")
    t0 = signal_ms(rec)
    if not coin:
        return "no_coin"
    ...
    # The gate is long-only (shorts pass through unrecorded); never grade
    # anything else defensively.
    if rec.get("side") not in (None, "long"):
        return "side_skip"
    try:
        entry = float(rec.get("price"))
    ...
    rec["entry_px"] = entry
    rec["pnl_pct"] = v
    rec["entry_px_source"] = "recorded_price"
...
def run(*, file: str = FILE, out: str = DEFAULT_OUT,
    ...
    for r in recs:
        ...
        if prior in _TERMINAL and not force:
            ...
            continue
        # Only vetoed signals are counterfactual trades; allowed signals
        # (trend_would_block False/None) have no hypothetical position.
        if r.get("trend_would_block") is not True:
            counts["skipped_not_counterfactual"] += 1
            continue
        t0 = signal_ms(r)
    ...
    if write and produced:
        ...
        # de-duping on (coin, ts, price); re-runs replace stale replays.
...
def main() -> int:
    ...
    ap.add_argument("--file", default=FILE, help="live trend_filter JSONL "
    ...
    print(f"skipped already-terminal: {stats['skipped_terminal']}  "
          f"not-counterfactual: {stats['skipped_not_counterfactual']}  "
